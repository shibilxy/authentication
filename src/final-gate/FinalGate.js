...
const handleReset = ()=>{
  ...
  useEffect(() => {
    Swal({
      title: 'GATE-2 UNLOCKED',
      text: 'Enter the password to unlock Gate-3',
      icon: 'success',
  ...
  const handleEnter = () => {
    ...
    playOpenSound();
    setIsRotated(true);
    if (inputValue === 'issathfarsana@123') {
      setIsEntered(true);
    } else {
      setTimeout(() => {
  ...
  return (
    <>
      <center style={{ marginTop: '50px',color:'white',  fontWeight: 'bold', fontSize: '24px' }}>
        GATE - <span style={{ color: 'green' }}>3/3</span>
      </center>
